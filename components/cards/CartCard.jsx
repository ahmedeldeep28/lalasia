...
function CartCard({ itemData }) {
  const { id, productName, quantity, color, image, price ,productId} = itemData;

  const dispatch = useDispatch();

  const removeItemTocart = () => {
    dispatch(deleteCart(id)).unwrap().then((res) => {
      toast.success("The product has been removed from the cart", {position: toast.POSITION.BOTTOM_RIGHT});
      ...
      <div className="col-span-12 sm:col-span-8 lg:col-span-9 flex flex-col justify-between">
        <div className="flex flex-wrap justify-between items-center">
          <Link href={`/product/${productId}`}>
            <a className="text-h3 hover:text-primary capitalize">{productName}</a>
          </Link>
          <Trash
