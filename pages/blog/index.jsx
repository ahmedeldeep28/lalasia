...
function blogs({articles,error}) {
  ...
  const articlesList = articles.map((article) => {
    return (
      <div className="col-span-12 md:col-span-6 lg:col-span-4" key={article.id}>
    ...
    />
      ...
      <section className="py-16">
        <div className="container">
          <div className="mb-4">
            <p className="text-h6 sm:text-h5 text-secondary mb-2">Trending Topics</p>
          ...
          <ul className="flex space-x-4 mb-4 overflow-auto	py-3">
            ...
            <li className="text-p2 sm:text-p1 text-base whitespace-nowrap duration-300 hover:text-title cursor-pointer">
              <Link href='/blog?category=tips and trick'>
               Tips and Trick
              </Link>
            </li>
            <li className="text-p2 sm:text-p1 text-base whitespace-nowrap duration-300 hover:text-title cursor-pointer">
            <Link href='/blog?category=interior design'>
              Interior Design
              </Link>
              </li>
            <li className="text-p2 sm:text-p1 text-base whitespace-nowrap duration-300 hover:text-title cursor-pointer">
            ...
            <li className="text-p2 sm:text-p1 text-base whitespace-nowrap duration-300 hover:text-title cursor-pointer">
            <Link href='/blog?category=color guide'>
             Color Guide
              </Link>
              </li>
          </ul>
          ...
          <>
          {articles.length === 0 ? 
            <p className="text-red-500 text-h5 p-5 bg-red-100 rounded">not found articles</p>
            :
...
export async function getServerSideProps({query}) {
  try {
    ...
    let articles =  await fetchApi(`blog${category ? `?category=${category}` : ''}`);
